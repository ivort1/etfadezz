      ...
      <div className="space-y-4">
        {services.map(({ name, src, price, duration, popular, link }, i) => (
          <div key={i} className={`relative rounded-xl px-4 py-8 border transition-all ${popular ? 'bg-linear-to-br from-blue-50 to-transparent border-blue-200' : 'bg-white border-slate-200'}`}>
            {popular && (
              <span className="absolute -top-2 right-4 bg-blue-600 text-white text-xs px-2 py-0.5 rounded-full font-medium">
                Popular
              </span>
            )}
            
            <div className="flex flex-row justify-between items-center">
              <img src={`/src/assets/services/${src}`} alt={`${name} icon`} className="w-12 h-12 mr-4 shrink-0" />
              ...
              <a href={link} className="text-blue-600 hover:text-blue-700 text-xs font-semibold tracking-wide transition-colors" rel="noreferrer">
                BOOK →
              </a>
            </div>
          </div>
